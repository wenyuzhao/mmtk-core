...
use super::lifo::{allocate_buffer, pack, unpack, StealError};
use super::lifo::{AtomicUnsignedLong, AtomicUnsignedShort, UnsafeCell, UnsignedShort};
    ...
    fn next(&mut self) -> Option<T> {
        ...
        if self.head == self.to_head {
            ...
            loop {
                ...
                debug_assert_eq!(stealer_head, self.from_head);
...
impl<T> Stealer<T> {
        ...
        C: FnMut(usize) -> usize,
    {
        ...
        loop {
            ...
            debug_assert_eq!(stealer_head, sh);
