...
pub(super) type UnsignedShort = u32;
pub(super) type UnsignedLong = u64;
pub(super) type AtomicUnsignedShort = std::sync::atomic::AtomicU32;
pub(super) type AtomicUnsignedLong = std::sync::atomic::AtomicU64;
...
pub(super) fn pack(value1: UnsignedShort, value2: UnsignedShort) -> UnsignedLong {
    ((value1 as UnsignedLong) << UnsignedShort::BITS) | value2 as UnsignedLong
}
#[inline]
/// Unpack a long integer into 2 short ones.
pub(super) fn unpack(value: UnsignedLong) -> (UnsignedShort, UnsignedShort) {
    (
...
pub(super) fn allocate_buffer<T>(len: usize) -> Box<[UnsafeCell<MaybeUninit<T>>]> {
...
impl<T> Stealer<T> {
        ...
        C: FnMut(usize) -> usize,
    {
        ...
        debug_assert!(dest_free_capacity <= dest.queue.capacity());
