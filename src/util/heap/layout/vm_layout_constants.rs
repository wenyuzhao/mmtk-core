...
impl VMLayoutConstants {
    ...
    pub fn new_64bit_with_pointer_compression(max_heap_size: usize) -> Self {
        assert!(
            max_heap_size <= (32usize << 30),
            "Heap size is larger than 32 GB"
        );
        let mut start: usize = 0x4000_0000;
        let mut end: usize = match start + max_heap_size {
        ...
        let heap_end = if cfg!(feature = "virt_constraint") {
            usize::min(
                ...
                (start + max_heap_size - 1 + BYTES_IN_CHUNK) & !(BYTES_IN_CHUNK - 1),
            )
        } else {
            end
        };
...
pub enum AddressSpaceKind {
    _32Bits,
    _64Bits,
    _64BitsWithPointerCompression { max_heap_size: usize },
}
